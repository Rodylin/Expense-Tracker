  ...
  <div class="justify-center items-center h-full">
    <div>
      ...
      <div class="flex flex-col justify-center mx-auto md:flex-row">
        <div
          class="lg:w-1/3 m-3 p-3 border border-slate-500 rounded bg-teal-200"
          style="height: 450px"
        >
          <form
          ...
          >
            ...
            <input
              type="text"
              v-model="name"
              class="border border-slate-500 rounded mb-5 p-2 bg-zinc-50"
            />

            <label for="category" class="my-5">Category: </label>
            <select
              v-model="category"
              class="mb-5 border border-slate-500 rounded p-2 bg-zinc-50"
            >
            ...
            <input
              type="number"
              v-model="amount"
              class="border border-slate-500 rounded mb-5 p-2 bg-zinc-50 bg-cyan-900"
            />
            <button
              type="submit"
              class="border-2 py-2 rounded text-black hover:opacity-80 hover:border-black hover:text-white hover:bg-black transition ease-linear duration-100 bg-slate-500"
            >
              {{ buttonText }}
            </button>
          </form>
        ...
        <div class="lg:w-2/3 m-3" style="max-height: 450px; width: 100%">
          <div>
            <table class="scrolldown">
              <thead>
                <tr>
                  <th class="bg-green-400">Date & Time</th>
                  <th class="bg-green-400">Item</th>
                  <th class="bg-green-400">Category</th>
                  <th class="bg-green-400">Amount</th>
                  <th class="bg-green-400">...</th>
                </tr>
              </thead>
              <tbody>
                <tr v-for="expense in expenses" :key="expense.id">
                  <td class="bg-green-950">
                    {{ formatDateTime(expense.data.datetime) }}
                  </td>
                  <td class="bg-emerald-700">
                    {{ expense.data.name }}
                  </td>
                  <td class="bg-green-950">
              ...
              <tfoot>
                <tr>
                  <td class="px-3 py-2 border-1 bg-slate-500">
...
<style>
body {
  background-image: url("https://miro.medium.com/v2/resize:fit:1400/0*32iyQIC08ao66YLu.gif");
}
table.scrolldown {
  height: 450px;
}
